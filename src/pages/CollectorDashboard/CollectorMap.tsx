...
export default function CollectorMap() {
  ...
  const activeAssignments = jobs.filter((j: any) => j.isAssignedToMe && j.status !== "Completed");
  const completedTodayList = jobs.filter((j: any) => j.isAssignedToMe && j.status === "Completed");
  ...
  const truckCoords = optimizedRoute.length > 0
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-4 bg-card border border-border px-4 py-2.5 rounded-xl shadow-sm">
          ...
          <span className="text-sm font-black text-[#10b981]">{completedTodayList.length} Completed</span>
        </div>
      </div>
                    ...
                    >
                      ...
                      <div className="pl-7 flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <Progress value={job.fill} className="w-16 h-1 bg-slate-100 dark:bg-slate-800 [&>[data-slot=progress-indicator]]:bg-[#ba1a1a]" />
                          <span className="text-[9px] font-bold text-[#ba1a1a]">{job.fill}% Full</span>
                        </div>

                        {/* Interactive HUD Trigger */}
                        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#f8fafc] dark:border-[#0f2942]" onClick={(e) => e.stopPropagation()}>
                          <button
                            onClick={() => {
                ...
                <div className="absolute top-3 right-3 z-10 bg-white/90 dark:bg-card/90 backdrop-blur-sm border border-slate-200 dark:border-border rounded-lg p-2 shadow-md flex flex-col gap-1.5">
                  ...
                  {[['#ef4444','Critical'],['#f59e0b','High'],['#3b82f6','Normal'],['#10b981','Completed']].map(([c,l])=>(
                    <div key={l} className="flex items-center gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-full border border-white/50" style={{backgroundColor:c as string}} />
                      <span className="text-[9px] text-slate-600 dark:text-slate-300 font-medium">{l}</span>
                    </div>
                  ))}
                  <div className="flex items-center gap-1.5 mt-0.5 pt-1.5 border-t border-slate-200 dark:border-slate-700">
                    <svg width="16" height="6"><line x1="0" y1="3" x2="16" y2="3" stroke="#0284c7" strokeWidth="2" strokeDasharray="3 2"/></svg>
                    <span className="text-[9px] text-slate-600 dark:text-slate-300 font-medium">Route</span>
                  </div>
                </div>
                <svg viewBox="0 0 600 400" className="w-full h-full rounded-xl overflow-hidden" style={{background:'#e8f0e4'}}>
                  <defs>
                    <style>{`
                    ...
                    <filter id="zs2" x="-10%" y="-10%" width="120%" height="120%">
                      <feDropShadow dx="0" dy="1" stdDeviation="2" floodColor="#00000015"/>
                    </filter>
                  </defs>

                  {/* Greenery background patches */}
                  {[[28,28],[572,28],[28,372],[572,372],[300,18],[300,382]].map(([cx,cy],i)=>(
                    <circle key={i} cx={cx} cy={cy} r="20" fill="#b8d4a0" opacity="0.65"/>
                  ))}

                  {/* Road ring */}
                  <rect x="68" y="58" width="464" height="284" rx="32" fill="none" stroke="#cdc8bc" strokeWidth="20"/>
                  {/* Cross paths */}
                  <line x1="300" y1="58" x2="300" y2="342" stroke="#cdc8bc" strokeWidth="14"/>
                  <line x1="68" y1="200" x2="532" y2="200" stroke="#cdc8bc" strokeWidth="14"/>
                  {/* Lane markings */}
                  <line x1="300" y1="58" x2="300" y2="342" stroke="#e8f0e4" strokeWidth="2.5" strokeDasharray="12 9"/>
                  <line x1="68" y1="200" x2="532" y2="200" stroke="#e8f0e4" strokeWidth="2.5" strokeDasharray="12 9"/>

                  {/* Zone A – top-left */}
                  <rect x="92" y="76" width="178" height="106" rx="12" fill="#dbeafe" stroke="#93c5fd" strokeWidth="1.5" filter="url(#zs2)"/>
                  <text x="181" y="120" textAnchor="middle" fontSize="13" fontWeight="bold" fill="#1d4ed8" fontFamily="sans-serif">Zone A</text>
                  <text x="181" y="137" textAnchor="middle" fontSize="9" fill="#3b82f6" fontFamily="sans-serif">North Wing</text>

                  {/* Zone B – top-right */}
                  <rect x="330" y="76" width="178" height="106" rx="12" fill="#fef3c7" stroke="#fcd34d" strokeWidth="1.5" filter="url(#zs2)"/>
                  <text x="419" y="120" textAnchor="middle" fontSize="13" fontWeight="bold" fill="#92400e" fontFamily="sans-serif">Zone B</text>
                  <text x="419" y="137" textAnchor="middle" fontSize="9" fill="#b45309" fontFamily="sans-serif">East Wing</text>

                  {/* Zone C – bottom-left */}
                  <rect x="92" y="218" width="178" height="106" rx="12" fill="#dcfce7" stroke="#86efac" strokeWidth="1.5" filter="url(#zs2)"/>
                  <text x="181" y="262" textAnchor="middle" fontSize="13" fontWeight="bold" fill="#15803d" fontFamily="sans-serif">Zone C</text>
                  <text x="181" y="279" textAnchor="middle" fontSize="9" fill="#16a34a" fontFamily="sans-serif">West Wing</text>

                  {/* Zone D – bottom-right */}
                  <rect x="330" y="218" width="178" height="106" rx="12" fill="#fce7f3" stroke="#f9a8d4" strokeWidth="1.5" filter="url(#zs2)"/>
                  <text x="419" y="262" textAnchor="middle" fontSize="13" fontWeight="bold" fill="#9d174d" fontFamily="sans-serif">Zone D</text>
                  <text x="419" y="279" textAnchor="middle" fontSize="9" fill="#be185d" fontFamily="sans-serif">South Wing</text>

                  {/* Central hub */}
                  <circle cx="300" cy="200" r="34" fill="#f1f5f9" stroke="#cbd5e1" strokeWidth="2" filter="url(#zs2)"/>
                  <text x="300" y="196" textAnchor="middle" fontSize="8.5" fontWeight="bold" fill="#475569" fontFamily="sans-serif">CENTRAL</text>
                  <text x="300" y="210" textAnchor="middle" fontSize="8.5" fontWeight="bold" fill="#475569" fontFamily="sans-serif">HUB</text>

                  {/* Depot */}
                  <rect x="528" y="179" width="56" height="22" rx="5" fill="#334155" filter="url(#ps2)"/>
                  <text x="556" y="194" textAnchor="middle" fontSize="8.5" fontWeight="bold" fill="white" fontFamily="sans-serif">DEPOT</text>

                  {/* Animated route */}
                  {activeAssignments.length > 0 && (
                    <path d={routePath} fill="none" stroke="#0284c7" strokeWidth="3.5"
                      strokeLinecap="round" strokeLinejoin="round"
                      strokeDasharray="10 6" className="route-anim" opacity="0.9"/>
                  )}
                  ...
                  {jobs.map((job: any) => {
                    ...
                    const isCurrentDest = activeAssignments.length > 0 && optimizedRoute.length > 0 && optimizedRoute[0].id === job.id;
                    const isSelected = selectedJobId === job.id;
                    const pc = isCompleted ? "#10b981" : job.urgency === "Critical" ? "#ef4444" : job.urgency === "High" ? "#f59e0b" : "#3b82f6";
                    return (
                      <g key={job.id} onClick={() => setSelectedJobId(job.id)} style={{cursor:'pointer'}}>
                        {(isSelected || isCurrentDest) && !isCompleted && (
                          <circle cx={coords.x} cy={coords.y} r="20" fill={pc} className="pin-glow"/>
                        )}
                        <g filter="url(#ps2)">
                          <path d={`M${coords.x},${coords.y+13} C${coords.x-9},${coords.y+3} ${coords.x-11},${coords.y-7} ${coords.x},${coords.y-13} C${coords.x+11},${coords.y-7} ${coords.x+9},${coords.y+3} ${coords.x},${coords.y+13}Z`}
                            fill={pc} stroke="white" strokeWidth={isSelected?"2.5":"1.5"}/>
                          <circle cx={coords.x} cy={coords.y-3} r="3.5" fill="white" opacity="0.9"/>
                        </g>
                        <rect x={coords.x-27} y={coords.y-31} width="54" height="13" rx="3" fill="white" stroke={pc} strokeWidth="1" opacity="0.95"/>
                        <text x={coords.x} y={coords.y-21} textAnchor="middle" fontSize="7" fill="#1e293b" fontWeight="bold" fontFamily="sans-serif">
                          {job.location.length > 13 ? job.location.slice(0,13)+'…' : job.location}
                        </text>
                      </g>
                    );
                  })}
                  ...
                  {/* Collector vehicle */}
                  {activeAssignments.length > 0 && (
                    <g transform={`translate(${truckCoords.x - 12}, ${truckCoords.y + 15})`} filter="url(#ps2)">
                      <rect x="0" y="0" width="24" height="13" rx="3" fill="#0284c7"/>
                      <rect x="15" y="2.5" width="9" height="7.5" rx="1" fill="#0369a1"/>
                      <circle cx="5.5" cy="13" r="3" fill="#1e293b"/><circle cx="18.5" cy="13" r="3" fill="#1e293b"/>
                      <circle cx="5.5" cy="13" r="1.4" fill="#94a3b8"/><circle cx="18.5" cy="13" r="1.4" fill="#94a3b8"/>
                      <rect x="1.5" y="2.5" width="7" height="5" rx="1" fill="#7dd3fc" opacity="0.85"/>
                    </g>
                  )}
                </svg>
              </div>
            ...
            {[...Array(30)].map((_, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ backgroundColor: randomColor, left: `${randomLeft}%` }}
                />
              );
            })}
          </motion.div>
              ...
              >
                <div className="flex justify-between items-center border-b border-[#f1f5f9] dark:border-[#0f2942] pb-3">
                  <div className="flex items-center gap-2 text-[#0284c7]">
                    ...
                    <span className="text-sm font-bold tracking-wider uppercase">Active HUD Navigation</span>
                  </div>
                  <button
                    onClick={() => {
