...
const getFillTextColor = (fill: number) => {
  ...
  return 'text-[#0b1c30] dark:text-white';
};
    ...
    >
      ...
      <div className="bg-white dark:bg-[#0b1c30] border border-[#e2e8f0] dark:border-[#1e3a5f] rounded-xl shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center w-full max-w-xs bg-white dark:bg-[#0b1c30] rounded-lg border border-[#e2e8f0] dark:border-[#1e3a5f] focus-within:border-[#cbd5e1] dark:border-[#334155] focus-within:shadow-sm transition-all overflow-hidden px-3 py-1.5">
              ...
              <input
                ...
                className="bg-transparent border-none outline-none text-xs font-medium text-[#0b1c30] dark:text-white placeholder-[#94a3b8] w-full"
                value={searchTerm}
            ...
            <button className="px-3 py-1.5 text-xs font-semibold border border-[#e2e8f0] dark:border-[#1e3a5f] rounded-lg hover:bg-white dark:bg-[#0b1c30] transition-all flex items-center gap-1 text-[#515f74] dark:text-[#cbd5e1]">
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-[#f8fafc] dark:bg-[#0f2942] border-b border-[#e2e8f0] dark:border-[#1e3a5f]">
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Bin Location</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Device ID</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Fill %</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Urgency</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Status</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider">Assigned</th>
                <th className="px-6 py-4 text-[11px] font-bold text-[#515f74] dark:text-[#cbd5e1] uppercase tracking-wider text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#f1f5f9]">
                ...
                <tr key={job.id} className="hover:bg-[#f8fafc] dark:bg-[#0f2942] transition-colors group">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[11px] text-[#515f74] dark:text-[#cbd5e1] mt-0.5">{job.zone}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <div className="w-16 h-1.5 bg-[#f1f5f9] dark:bg-[#1a365d] rounded-full overflow-hidden">
                    ...
                    {job.assignedTo ? (
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-[#e2e8f0] flex items-center justify-center shrink-0">
                          <span className="text-[9px] font-bold text-[#515f74] dark:text-[#cbd5e1]">{getInitials(job.assignedTo)}</span>
                        </div>
                        <span className="text-sm text-[#0b1c30] dark:text-white">{job.assignedTo}</span>
                      </div>
                    ) : (
                      <span className="text-[#94a3b8] dark:text-[#64748b] italic text-xs">Unassigned</span>
                    )}
                  </td>
              ...
              {filteredData.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-[#94a3b8] dark:text-[#64748b] text-sm">
        ...
        <div className="px-6 py-4 border-t border-[#f1f5f9] dark:border-[#0f2942] bg-[#f8fafc] dark:bg-[#0f2942]/50 flex items-center justify-between mt-auto">
          <span className="text-xs text-[#515f74] dark:text-[#cbd5e1]">
            Showing <span className="font-bold text-[#0b1c30] dark:text-white">{filteredData.length}</span> of {jobs.length} active jobs
          </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-[#0b1c30] rounded-xl p-6 flex items-center justify-between overflow-hidden relative shadow-lg">
          <div className="relative z-10 space-y-3 max-w-lg">
            ...
            <p className="text-[#94a3b8] dark:text-[#64748b] text-sm leading-relaxed">
        ...
        {/* Facility Load */}
        <div className="bg-white dark:bg-[#0b1c30] border border-[#e2e8f0] dark:border-[#1e3a5f] rounded-xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 text-[#006c49]">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" />
            </svg>
            <h4 className="font-bold text-sm text-[#0b1c30] dark:text-white">Facility Load</h4>
          </div>
          <div className="space-y-3">
            <div className="flex justify-between text-xs font-medium text-[#0b1c30] dark:text-white">
          ...
          <div className="space-y-3 pt-3 border-t border-[#f1f5f9] dark:border-[#0f2942]">
            <div className="flex justify-between text-xs font-medium text-[#0b1c30] dark:text-white">
      ...
      {assigningJobId && (
        <div className="fixed inset-0 bg-[#0b1c30]/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0b1c30] rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="p-4 border-b border-[#e2e8f0] dark:border-[#1e3a5f] flex justify-between items-center bg-[#f8fafc] dark:bg-[#0f2942]">
              <h3 className="font-bold text-[#0b1c30] dark:text-white">Assign Collector</h3>
              <button onClick={() => setAssigningJobId(null)} className="text-[#94a3b8] dark:text-[#64748b] hover:text-[#0b1c30] dark:text-white transition-colors">
            ...
            <div className="p-2 max-h-[300px] overflow-y-auto flex flex-col gap-1">
              {AVAILABLE_COLLECTORS.map(collector => (
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-[#0b1c30] dark:text-white">{collector}</span>
                </button>
              ))}
            </div>
          </div>
