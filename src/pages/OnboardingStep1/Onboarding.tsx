...
export default function Onboarding() {
  ...
  const validate = () => {
    ...
    if (form.role === "Select a role" || !form.role)
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="bg-white dark:bg-[#0b1c30] border border-[#e2e8f0] dark:border-[#1e3a5f] rounded-2xl shadow-sm p-6 sm:p-8 flex flex-col gap-5">
            ...
            <SelectField
              ...
              onChange={set("role")}
              error={errors.role}
              options={["Select a role", "Manager", "Collector"]}
            />
              ...
              <div className="flex items-center justify-between">
                <StepDots current={0} total={2} />
                 <p className="text-xs text-[#94a3b8] dark:text-slate-400">
                  ...
                  <Link
                    to="/login"
                    className="text-[#006c49] dark:text-emerald-400 font-medium hover:underline dark:hover:text-emerald-300"
                  >
                    Log in
                  </Link>
                </p>
