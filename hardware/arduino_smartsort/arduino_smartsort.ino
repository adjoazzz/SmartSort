...
#define TRIG_PIN 4
#define ECHO_PIN 12
#define TRIGGER_DISTANCE_CM 7.0
...
void setup() {
  ...
  // Restore last known position from EEPROM (survives power loss)
  if (EEPROM.read(EEPROM_MAGIC_ADDR) == EEPROM_MAGIC_VALUE) {
    EEPROM.get(EEPROM_ANGLE_ADDR, currentAngle);
    Serial.print("EEPROM: Last saved position = ");
    ...
    // First-ever run: initialize EEPROM with home position
    currentAngle = HOME_ANGLE;
    EEPROM.write(EEPROM_MAGIC_ADDR, EEPROM_MAGIC_VALUE);
    saveAngleToEEPROM(currentAngle);
    Serial.println("EEPROM: First run detected, initialized to 0 degrees.");
  }
  ...
  flapServo.write(
      FLAP_OPEN_DEG); // Assume it's dropped open at 0 when powered off
  delay(100);
...
void loop() {
  ...
  if (espSerial.available()) {
    ...
    if (incoming.startsWith("SORT:")) {
      ...
      lastTriggerTime = millis(); // Reset cooldown AFTER sort cycle finishes
...
void handleSortCommand(String category) {
  ...
  if (abs(angleDiff) > 0.1) {
    ...
    saveAngleToEEPROM(currentAngle); // Persist position in case of power loss
  } else {
  ...
  if (abs(currentAngle - HOME_ANGLE) > 0.1) {
    ...
    stepper.step(stepsToHome);
    currentAngle = HOME_ANGLE;
    saveAngleToEEPROM(currentAngle); // Persist home position
